...
func main() {
	...
	fmt.Println("Lendo o arquivo CSV dos discentes...")
	...
	for i, authorRecord := range authorRecords {
		achado = 0
		for _, studentRecord := range studentNames {
			...
			for _, authorName := range authorNames {
				...
				if similarity > 0.85 {
					achado = 1
					msg := fmt.Sprintf("%03d | Similaridade %.2f entre: %-40s | %-40s | Artigo: %000d", totalSimilarities, similarity, authorName, studentName, i)
					progress <- msg
					// clearScreen()
				}
			}
