...
'''
Implementação detalhada para monitorar threads e processos na classe DatasetArticlesGenerator. Registra o estado inicial do sistema antes dos experimentos e identifica mudanças durante e após a execução dos experimentos, para isolar as threads e medir quantitativamente os tempos de execução e recursos consumidos.

Esta implementação permite que a classe DatasetArticlesGenerator monitore threads e processos de forma eficiente. Os métodos start_thread_monitoring e start_process_monitoring iniciam o monitoramento de threads e processos, respectivamente, enquanto stop_thread_monitoring e stop_process_monitoring param este monitoramento. O método finalize_monitoring consolida os dados coletados, fornecendo uma visão clara das threads e processos criados e remanescentes após a conclusão dos experimentos. Este design ajuda a identificar recursos adicionais que foram iniciados durante a execução dos experimentos e que podem estar interferindo nas métricas de desempenho. Esta biblioteca oferece uma interface robusta e multiplataforma para obter informações sobre os processos do sistema operacional.
...
class DatasetArticlesGenerator:
    def __init__(self, base_repo_dir):
        ...
        self.gpu_transfer_time = 0.0  # Inicializar acumulador de tempo de transferência para GPU
        self.gpu_load_time = 0  # Tempo de carga para GPU
        self.gpu_calc_time = 0  # Tempo de cálculo na GPU
        self.cpu_calc_time = 0  # Tempo de cálculo na CPU
    ...
    def convert_list_to_dict(self, lst: List[Any]) -> Dict[str, Any]:
        # Converter uma lista em um dicionário com índices como chaves
        return {str(i): item for i, item in enumerate(lst)}

    def preprocess_data(self, extracted_data: Dict[str, Any]) -> Dict[str, Any]:
        # Pré-processar os dados extraídos para transformar em dicionários
    ...
    def _recursive_preprocessing(self, data: Dict[str, Any]) -> Dict[str, Any]:
        # Pré-processar recursivamente um dicionário
    ...
    def extrair_idlattes(self, dic: Dict[str, Any]) -> Optional[str]:
        # Extrair o ID Lattes de um dicionário
        try:
            id_lattes_info = dic.get('InfPes', {}).get('2', '') or dic.get('InfPes', {}).get('3', '')
            return self._buscar_id_lattes(id_lattes_info)
    ...
    def calculate_jaccard_similarity_cpu(self, str1, str2):
        # Iniciar cronômetro para cálculo na CPU
    ...
    def calculate_jaccard_similarity_gpu(self, str1, str2):
        if torch.cuda.is_available():
            # Inicializar cronômetros
            start_time = time.time()
            gpu_load_time_start = time.time()

            self.logger.info("Iniciando cálculo de similaridade Jaccard com GPU")
            device = torch.device("cuda")
            torch.cuda.synchronize()  # Sincronização antes de iniciar operações

            # Preparar dados para transferência para a GPU
            set1 = set(str1.split())
            set2 = set(str2.split())
            ...
            # Registrar o tempo de carregamento para a GPU
            gpu_load_time_end = time.time()
            self.gpu_load_time += gpu_load_time_end - gpu_load_time_start

            # Iniciar cronômetro para cálculo na GPU
            gpu_calc_time_start = time.time()

            # Executar cálculo na GPU
            intersection = torch.sum(vec1 * vec2).float()
            union = torch.sum(vec1 + vec2 - (vec1 * vec2)).float()
            similarity = intersection / union if union != 0 else 0

            # Registrar o tempo de cálculo na GPU
            gpu_calc_time_end = time.time()
            self.gpu_calc_time += gpu_calc_time_end - gpu_calc_time_start

            torch.cuda.synchronize()  # Sincronização após as operações
            end_time = time.time()
            self.logger.info(f"Tempo total para cálculo de similaridade Jaccard com GPU: {end_time - start_time:.4f} segundos")
    ...
    @staticmethod
    @jit(nopython=True)
    def calculate_jaccard_similarity_numba(set1, set2):
    ...
    def _basic_data_processing(self, extracted_data: Dict, use_gpu=False) -> Optional[Dict]:
        self.logger.info(f"Processing single result, use_gpu={use_gpu}")
        try:
            ...
            # print('jcr2_articles:',jcr2_articles)

            # Processar cada artigo em Produções
            for key, producoes_article in producoes_articles.items():
                ...
                if key in jcr2_articles:
                    # Complementar com dados de JCR2, se existirem
                    # print('Adicionar a JCR2:',{key: jcr2_articles[key]})
            ...
            for article in articles_info:
                for key, art_info in article.items():
                    # Preencher dados com CrossRef
                    if 'doi' in art_info and art_info['doi']:
                        title, abstract, request_count = self.fetch_article_info_from_crossref(art_info['doi'])
                        if title != "Title not found" and title != "Error":
                            art_info['titulo'] = title
                            art_info['abstract'] = abstract

                    # Preenchimento de dados faltantes via scraping
                    elif not art_info.get('titulo') or not art_info.get('abstract'):
                        if 'doi' in art_info and art_info['doi']:
                            doi = art_info['doi'].replace('http://dx.doi.org/', '')
                            scraped_title, scraped_abstract = self.scrape_article_info(doi)
            ...
            areas_of_expertise_data = self._extract_areas_of_expertise(preprocessed_data.get('Áreas de atuação', {}))

            return {
                'id_lattes': id_lattes,
                'name': preprocessed_data.get('name', 'N/A'),
                'areas_of_expertise': areas_of_expertise_data,
    ...
    def _fill_missing_data(self, articles_info, use_gpu=False):
        ...
        via scraping ou CrossRef. Utiliza GPU para cálculos de similaridade, se disponível.
    ...
    def process_single_result(self, extracted_data: Dict) -> Optional[Dict]:
        # Inicia o monitoramento
        self.start_thread_monitoring()
        self.start_process_monitoring()

        # Processamento básico dos dados em CPU
    ...
    def process_single_result_multiprocess(self, args):
        """
        Método auxiliar para executar a função process_single_result em paralelo.
        """
        extracted_data, _, _ = args  # Ajustado para extrair apenas o primeiro argumento
        return self.process_single_result(extracted_data)
    ...
    def process_dicts(self, all_extracted_data: List[Dict], 
                      json_filename: str, 
        ...
        for extracted_data in tqdm(all_extracted_data, 
            ...
            processed_data = self.process_single_result(extracted_data)
    ...
    def process_dicts_with_gpu(self, all_extracted_data: List[Dict], 
        ...
        # for article in tqdm(successful_processed_data, desc="Fetching additional info"):
    ...
    def process_dicts_multithread(self, all_extracted_data: List[Dict], 
        ...
        with ThreadPoolExecutor(max_workers=os.cpu_count()) as executor:
            ...
            futures = [executor.submit(self.process_single_result, data) for data in all_extracted_data]
            ...
            for future in tqdm(as_completed(futures), total=len(futures), 
                ...
                try:
                    ...
                    # Armazenar a exceção ou alguma informação de erro para debug.
    ...
    ## Sem feedback tqdm
    # def process_dicts_multithread(self, all_extracted_data, json_filename, hdf5_filename):
    #     with ThreadPoolExecutor() as executor:
    #         futures = [executor.submit(self.process_single_result, data, json_filename, hdf5_filename) for data in all_extracted_data]
    ...
    def process_data_multithreads_with_go_optim(self,
        ...
        if go_executable is None:
            if os_type == "Linux":
                go_executable = os.path.join(self.folder_domain, "dataset_articles_generator_optim_linux")
            elif os_type == "Windows":
                go_executable = os.path.join(self.folder_domain, "dataset_articles_generator_optim_windows.exe")
            else:
                raise Exception("Sistema operacional não suportado")
        ...
        input_json = os.path.join(self.folder_data_input, "normalized_dict_list.json")
        output_json = os.path.join(self.folder_data_output, "output_go_cpu_multithreads_optim.json")
        ...
        try:
            ...
            def update_progress():
                progress_bar = tqdm(total=len(dict_list), desc="Multithreads CPU-DsPrep on Goroutine-Semaphors")
                while go_process.poll() is None:
                    try:
                        with open(progress_file, "r") as file:
                            progress = int(file.read().strip())
    ...
    # def process_dicts_multiprocess(self, all_extracted_data: List[Dict], json_filename: str, hdf5_filename: str) -> List[Dict]:
    #     successful_processed_data = []
    ...
    #     tasks = [(data, json_filename, hdf5_filename) for data in all_extracted_data]
    ...
    def process_dicts_multiprocess(self, all_extracted_data, json_filename, hdf5_filename, progress_callback=None):
        successful_processed_data = []
    ...
    def plot_comparision(self, single_thread_time, 
                         single_thread_gpu_time,
                         multi_thread_time, 
                         multi_thread_gpu_time, 
                         multi_process_time, 
                         multi_process_gpu_time):
        # Criar gráfico de barras com rótulos
        fig = go.Figure(data=[
            go.Bar(
                name='Single-thread',
                x=['Without GPU', 'With GPU'],
                y=[single_thread_time, single_thread_gpu_time],  # Lista de tempos
                text=[f"{single_thread_time:.2f}", f"{single_thread_gpu_time:.2f}"],  # Rótulos formatados
                textposition='auto',
            ...
            go.Bar(
                name='Multi-thread',
                x=['Without GPU', 'With GPU'],
                y=[multi_thread_time, multi_thread_gpu_time],  # Lista de tempos
                text=[f"{multi_thread_time:.2f}", f"{multi_thread_gpu_time:.2f}"],  # Rótulos formatados
                textposition='auto',
                textfont=dict(size=32),
            ...
            go.Bar(
                name='Multi-process',
                x=['Without GPU', 'With GPU'],
                y=[multi_process_time, multi_process_gpu_time],  # Tempos com multiprocessing
                text=[f"{multi_process_time:.2f}", f"{multi_process_gpu_time:.2f}"],  # Rótulos formatados
                textposition='auto',
                textfont=dict(size=32),
        ...
        # Exibir gráfico em célula do notebook jupyter
        fig.show(renderer="notebook")

        return fig

    def plot_scatterplot_comparision(self, results):
        # Converter resultados em listas separadas para cada configuração
        single_thread_times = [res[0] for res in results]
        single_thread_gpu_times = [res[1] for res in results]
        multi_thread_times = [res[2] for res in results]
        multi_thread_gpu_times = [res[3] for res in results]
        multi_process_times = [res[4] for res in results]
        multi_process_gpu_times = [res[5] for res in results]
        ...
        # Configurar layout
        fig.update_layout(title='Comparação visual dos Tempos de Execução',
    ...
    def plot_boxplot_comparision(self, results):
        # Desempacotar tempos de execução
        single_thread_times = [result[0] for result in results]
        single_thread_gpu_times = [result[1] for result in results]
        multi_thread_times = [result[2] for result in results]
        multi_thread_gpu_times = [result[3] for result in results]
        multi_process_times = [result[4] for result in results]
        multi_process_gpu_times = [result[5] for result in results]
        ...
        # Atualizar layout
        fig.update_layout(height=600, width=1800, title_text="Comparação estatística dos Tempos de Execução em segundos")

        fig.show(renderer="notebook")

    ## Função de comparação substituída pela classe especializada em Monitoramento
    def compare_execution_times(self, all_extracted_data, json_filename, hdf5_filename):
        self.logger.info("Comparing execution times")        
        print("1. Medir tempo de execução serial em CPU")
        start_time = time.time()
        self.process_dicts(all_extracted_data, json_filename, hdf5_filename)
        single_thread_time = time.time() - start_time

        print("2. Medir tempo de execução serial com GPU")
        start_time = time.time()
        self.process_dicts_with_gpu(all_extracted_data, json_filename, hdf5_filename)
        single_thread_gpu_time = time.time() - start_time

        print("3. Medir tempo de execução com multithreading em CPU")
        start_time = time.time()
        self.process_dicts_multithread(all_extracted_data, json_filename, hdf5_filename)
        multi_thread_time = time.time() - start_time

        print("4. Medir tempo de execução com multithreading e com GPU")
        start_time = time.time()
        self.process_dicts_multithreads_with_gpu(all_extracted_data, json_filename, hdf5_filename)
        multi_thread_gpu_time = time.time() - start_time

        print("5. Medir tempo de execução com multiprocessing em CPU com Python")
        start_time = time.time()
        self.process_dicts_multiprocess(all_extracted_data, json_filename, hdf5_filename)
        multi_process_time = time.time() - start_time

        print("6. Medir tempo de execução com multiprocessing em CPU com Golang, Goroutines e Semáforos")
        start_time = time.time()
        self.process_dicts_multiprocess_gpu(all_extracted_data, json_filename, hdf5_filename)
        multi_process_gpu_time = time.time() - start_time

        # Plotar os tempos de execução com Plotly
        fig = go.Figure(data=[
            go.Bar(name='Single-thread', x=['Without GPU', 'With GPU'], 
                y=[single_thread_time, single_thread_gpu_time]),
            go.Bar(name='Multi-thread', x=['Without GPU', 'With GPU'], 
                y=[multi_thread_time, multi_thread_gpu_time]),
            go.Bar(name='Multi-process', x=['Without GPU', 'With GPU'], 
                y=[multi_process_time, multi_process_gpu_time])
        ])
        
        fig.update_layout(barmode='group', title='Comparison of Execution Times', 
                        yaxis_title='Execution Time (seconds)')
        fig.show(renderer="notebook")
    ...
    def plot_time_spent_by_activity(self, experiment_data):
        ...
        scraping_times = [data['time_spent'].get('scraping', 0) for data in experiment_data]
        crossref_times = [data['time_spent'].get('crossref', 0) for data in experiment_data]
        data_processing_times = [data['time_spent'].get('data_processing', 0) for data in experiment_data]
        gpu_load_times = [data['time_spent'].get('gpu_load', 0) for data in experiment_data]
        gpu_calc_times = [data['time_spent'].get('gpu_calc', 0) for data in experiment_data]
        cpu_calc_times = [data['time_spent'].get('cpu_calc', 0) for data in experiment_data]

        fig = go.Figure(data=[
            go.Bar(name='Scraping', x=labels, y=scraping_times),
            go.Bar(name='CrossRef', x=labels, y=crossref_times, base=scraping_times),
            go.Bar(name='Data Processing', x=labels, y=data_processing_times, base=[x + y for x, y in zip(scraping_times, crossref_times)]),
            go.Bar(name='GPU Load', x=labels, y=gpu_load_times, base=[x + y + z for x, y, z in zip(scraping_times, crossref_times, data_processing_times)]),
            go.Bar(name='GPU Calc', x=labels, y=gpu_calc_times, base=[x + y + z + w for x, y, z, w in zip(scraping_times, crossref_times, data_processing_times, gpu_load_times)]),
            go.Bar(name='CPU Calc', x=labels, y=cpu_calc_times, base=[x + y + z + w + v for x, y, z, w, v in zip(scraping_times, crossref_times, data_processing_times, gpu_load_times, gpu_calc_times)])
        ])

        fig.update_layout(barmode='stack', title='Tempo Gasto por Atividade em Cada Experimento')
        fig.show(renderer="notebook")